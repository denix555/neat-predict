...
def transition(price,):
    ...
    if not data:
        ...
        data.append(trade)
        data[-1]['high'] = high_level(data[-1]['high'],n_truncate[pair])
...
def run(price):
    ...
    if not data or (data and price <= data[-1]['low']):
        ...
        logger(time.ctime(time.time()) + ": " + str(data) + '\n')


def run_bnb(price):
    global pair,part,wait,n_truncate,data
    ...
    if not data:
        ...
        logger("run_bnb(): " + time.ctime(time.time()) + ": " + str(data) + '\n')
        ...
        if wait % 60 == 0:
            data[-1]['low'] = high_level(data[-1]['low'],n_truncate[pair])
            logger("run_bnb(): " + time.ctime(time.time()) + ": " + str(data) + '\n')
