...
def low_level(price,n,data):
    result = 0.0

    actions = len(data)
    result = truncate(price - (price / 100 * 0.20 * (actions + 1)), n)
...
def create_trade(price,data,n=n_truncate[pair]):
    trade = {}

    trade['buy'] = price
    trade['high'] = high_level(price,n,data)
    trade['low'] = low_level(price,n,data)
...
def run(price):
    global limit,part,data

    if len(data) == 0:
        balance = get_balance("USDT")
        part = balance / limit
        result = buy()
        trade = create_trade(result,data)
        data.append(trade)
        ...
        if price < data[-1]['low'] and len(data) < limit:
        ...
        elif price < data[-1]['low'] and len(data) == limit:
            time.sleep(600)
            ...
            mean_high = data[-1]['high'] - data[-1]['high'] / 100 * 0.25
            profit = take_profit(mean_high,result)
            data.remove(data[-1])
            ...
            if len(data) == 1:
                ...
                mean_buy = mean_price(data[0]['buy'],data[-1]['buy'])
                data[-1]['high'] = mean_buy + mean_buy / 100 * 0.25
