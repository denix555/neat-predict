...
################################### DATA ######################################
    ...
    ins = ['sma1','cho310','rsi7','cci7','arn_up','arn_down']

    g = [[1, 2, 1], [2, 2, 0], [2, 4, 0], [5, 4, 0], [1, 2, 1], [3, 2, 0], [0, 5, 0], [0, 2, 0], [1, 4, 0], [3, 3, 1], [5, 5, 0], [1, 5, 0], [1, 2, 0], [0, 1, 0], [1, 1, 1], [2, 2, 0], 20]
...
################################## PREPARE ##################################
    ...
    if klines:
        ...
        for i in range(len_d):
            ...
            k['arn_up'] = aroon([klines[x]['close'] for x in range(i,i+arn25)])
            k['arn_down'] = aroon([klines[x]['close'] for x in range(i,i+arn25)],m='down')
            k['rsi7'] = rsi([klines[x]['close'] for x in range(i,i+rsi7)][::-1])
            k['cci7'] = cci([sum([klines[x]['high'],klines[x]['low'],klines[x]['close']]) / 3 for x in range(i,i+cci7)][::-1])
...
################################### TRADE #####################################
        ...
        if not buy:
            ...
            if comp(ins[g[0][0]],g[0][1],g[0][2],d) and \
               ...
               (comp(ins[g[2][0]],g[2][1],g[2][2],d) or \
